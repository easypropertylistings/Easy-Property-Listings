	...
	if ( !empty($links) ) {
		foreach ( $links as $k=>$link ) {
			if(!empty($link)) {
				...
				<button type="button" class="epl-button epl-floor-plan" onclick="location.href='<?php echo $link; ?>'"><?php echo apply_filters( 'epl_button_label_floorplan' , __('Floor Plan', 'easy-property-listings') ) . ' ' . $number_string; ?></button></span><?php
			}
		}
	}
}
