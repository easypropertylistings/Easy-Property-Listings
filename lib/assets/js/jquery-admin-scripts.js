jQuery(document).ready(
        function ($) {
                ...
                var EPL_Admin = {
                        ...
                        /**
                         * @since 3.5.4 Show warning when display address is unchecked.
                         */
                        display_address_note: function() {

                                jQuery(document).find('#property_address_display_yes').on('change', function() {
                                        
                                        jQuery('.epl-display-address-warning').remove();
                                        
                                        if (!jQuery(this).is(':checked')) {
                
                                                var warningMessage = '<div class="notice notice-warning epl-display-address-warning" style="margin:10px 0;"><p>' + epl_admin_vars.display_address_note + '</p></div>';
                                                jQuery('#epl_property_address_coordinates').after(warningMessage);
                                ...
                                setTimeout( function() {
                                        jQuery(document).find('#property_address_display_yes').trigger('change');
                                }, 1000);
                ...
                if ($("#property_inspection_times").length) {
                        ...
                        function generateAllowedTimes() {
                                ...
                                for (let h = 0; h <= 11; h++) {
                                    for (let m = 0; m < 60; m += 5) {
                                        const hour = h.toString().padStart(2, '0'); // Ensures two digits for hours
                                        const minute = m.toString().padStart(2, '0'); // Ensures two digits for minutes
                        ...
                        jQuery('#epl-inspection-date').epl_datetimepicker({ 'timepicker': false, 'format': 'd-M-Y', 'closeOnDateSelect': true, 'allowBlank': false, inline: true, defaultDate:new Date() });
                        jQuery('#epl-inspection-start-time').epl_datetimepicker({ 'datepicker': false, 'format': 'h:i', 'hours12': true, allowTimes : generateAllowedTimes() });
                        jQuery('#epl-inspection-end-time').epl_datetimepicker({ 'datepicker': false, 'format': 'h:i', 'hours12': true, allowTimes : generateAllowedTimes() });
                ...
                $(document).on(
                        ...
                        function (e) {
                            ...
                            $('.epl-inspection-msg').html('');
                    
                            // Get input values
                            var inspectionDate = $('#epl-inspection-date').val();
                            var startTime = $('#epl-inspection-start-time').val();
                            var startAmPm = $('#epl-inspection-start-ampm').val();
                            var endTime = $('#epl-inspection-end-time').val();
                            var endAmPm = $('#epl-inspection-end-ampm').val();
                    
                            // Validation
                            if (!inspectionDate || !startTime || !startAmPm || !endTime || !endAmPm) {
                                $('.epl-inspection-msg').html('All fields are required. Please fill out all inspection details.');
                                return;
                            }
                    
                            // Make inspection time as per format
                            var added = inspectionDate + ' ' + startTime + startAmPm + ' to ' + endTime + endAmPm;
                            ...
                            if ($.inArray(added, eplAddedInspection) == -1) {
                                ...
                                var newInspection = $('<span><span class="epl-inspection-text">' + added + '</span><span class="del-inspection-time">&times;</span></span>');
                                eplAddedInspection = eplAddedInspection.join('\n');
                                $("#property_inspection_times").val(eplAddedInspection);
                                newInspection.hide().appendTo('.epl-added-inspection').fadeIn(600);
                            } else {
                                $('.epl-inspection-msg').html('Duplicate inspection entry.');
                                return;
                            }
                        }
                );
                ...
                */
                ...
                let EPL_Image_Manager = {
                        ...
                        make_sortable : function() {
                                ...
                                if ($(".epl-listing-attachments-list").length) {

                                        $(".epl-listing-attachments-list").each(function() {
                        ...
                        add_image : function() {
                                ...
                                if ($('.epl-listing-images-upload-btn').length > 0) {

                                        if (typeof wp !== 'undefined' && wp.media && wp.media.editor) {

                                            $(document).on('click', '.epl-listing-images-upload-btn', function(e) {

                                                e.preventDefault();
                                                var button = $(this);
                                                var mediaFrame = wp.media({
                                                        title: epl_admin_vars.add_images_button_text,
                                                        frame: "post",
                                                        multiple: true,
                                                        library: {
                                                                type: "image",
                                                ...
                                                mediaFrame.on("close",function() {
                                                        ...
                                                        var parentDiv = button.closest('.epl-listing-attachments-list-wrap').find('.epl-listing-attachments-list');
                                                        var extension = parentDiv.data('extension');
                                                        var prefix    = parentDiv.data('prefix');  
                                                        ...
                                                        selection.each(function(attachment) {
                                                                        ...
                                                                        var tpl = `
                                                                                ...
                                                                                <li data-id="`+attachment.id+`"  class="ui-state-default epl-listing-attachments-list-item `+prefix+`admin_thumb">

                                                                                        <span class="epl-slider-unattach">
                                                                                                <div class="epl-radio-switch">
                                                                                                <input 
                                                                                                        id="epl-`+extension+`-cmn-toggle-`+attachment.id+`" 
                                                                                                        class="epl-radio-switch-input epl-radio-switch-input--yes-no" 
                                                                                                        name="`+prefix+`enabled_thumbs[]" 
                                                                                                        checked="checked"
                                                                                                        value="`+attachment.id+`" 
                                                                                                        type="checkbox"
                                                                                                />
                                                                                                <label for="epl-`+extension+`-cmn-toggle-`+attachment.id+`"></label>
                                                                                                </div>
                                                                                        </span>

                                                                                        <div class="epl-`+extension+`-slide-tools epl-listing-attachment-tools">
                                                                                                <a target="_blank" href="post.php?post=`+attachment.id+`&action=edit">
                                                                                                <span class="epl-`+extension+`-edit-attach dashicons dashicons-edit"></span>
                                                                                                </a>
                                                                                                <a href="#" class="epl-slider-`+extension+`-delete-attach">
                                                                                                <span class="dashicons dashicons-trash"></span>
                                                                                                </a>
                                                                                        </div>
                                                                                        
                                                                                        <img src="`+attachment.sizes['admin-list-thumb'].url+`">
                                                                                </li>
                                                                        `;
                                                                        parentDiv.append(tpl);
                                                        });
                                                });
                                                mediaFrame.open(button);
                                                return false;
                                            });
                                        }
                                }
                        ...
                        delete_image : function() {
                                ...
                                $(document).on( 'click', '.epl-listing-attachment-tools-delete-attach', function(e) {
                                        e.preventDefault();
                            
                                        let parent = $(this).closest( '.epl-listing-attachments-list-item' );
                                        let attach_id = parent.data( 'id' );
                                        
                                        var parentDiv = $(this).closest('.epl-listing-attachments-list');
                                        var extension = parentDiv.data('extension');
                                        var prefix    = parentDiv.data('prefix');  
                ...
                EPL_Image_Manager.init();

                jQuery(document).on( 'click', '.epl-update-inspection-times', function (e) {
                        e.preventDefault();
                        jQuery('.epl-inspection-markup').slideToggle();
                });
        }
...
function epl_generate_inspection_markup() {
        ...
        tpl = '<div class="epl-inspection-msg"></div><div class="epl-added-inspection"></div><a class="epl-update-inspection-times" href="#">' + epl_admin_vars.inspection_times + '</a><div id="epl-inspection-markup" class="epl-inspection-markup"> <div class="epl-inspection-markup-inner">';
        tpl += '<div class="epl-inspection-col"> <div class="epl-inspection-field-label"> Date </div> <div class="epl-inspection-col-items"> <input type="text" autocomplete="off" id="epl-inspection-date" maxlength="2" size="2" placeholder="01"></div></div>';
        tpl += '<div class="epl-inspection-col"> <div class="epl-inspection-field-label"> Start </div>';
        tpl += '<div class="epl-inspection-col-items"><div class="epl-inspection-col-item">';
        tpl += '<input class="" type="text" autocomplete="off" id="epl-inspection-start-time" placeholder="01:05">';
        ...
        tpl += '</select>';
        tpl += ' <div class="epl-inspection-field-label"> End </div> </div><div class="epl-inspection-col-item">';
        tpl += '<input class="" type="text" autocomplete="off" id="epl-inspection-end-time" placeholder="01:05">';

        tpl += '<select id="epl-inspection-end-ampm" class="epl-inspection-ampm">';
        tpl += '<option value="AM">AM</option>';
        tpl += '<option value="PM">PM</option>';
        tpl += '</select></div><div class="epl-inspection-col-item epl-inspection-controls"><a id="epl-inspection-add" class="button">Add</a></div></div></div>';
