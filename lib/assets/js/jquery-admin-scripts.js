jQuery(document).ready(function($) {
	...
	var EPL_Admin = {
		...
		mark_featured : function() {
			...
			$('.column-property_featured span').on('click', function(e) {
	...
	if($( ".epl-upgrade-btn" ).length){
		...
		$('.epl-upgrade-btn').on('click', function(e) {
			e.preventDefault();

			var upgrade_ver = $(this).data('upgrade');
			var btn = $('this');
			var postData = {
			...
			$.post(ajaxurl, postData, function( response ) {
				if(response.status == 'success') {
					$('.epl-ajax-notice').addClass(' ajax-success').html(response.msg);
				} else {
					$('.epl-ajax-notice').addClass(' ajax-fail').html(response.msg);
				}
			}, 'json');
		});
	...
	if( $('.epl-widget-author-username').length > 0 ) {
		...
		$('.epl-widget-author-username').keyup(function(e) {
	...
	if( $('#property_second_agent').length == 1 || $('#property_agent').length == 1 ) {
		...
		$('#property_second_agent,#property_agent').keyup(function(e) {
			var currTarget = e.currentTarget;
			document.body.style.cursor='progress';
			if($(currTarget).val() == '') {
				$('.epl-contact-user-suggestion').fadeOut();
