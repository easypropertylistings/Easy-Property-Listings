	...
	*/
    ...
    $('.contact_category_suggestions > li >  a').on('click',function(e) {
        ...
        $.ajax({
            ...
            success: function (success) {
                if(success) {
                    $('.contact-entry-header span').text(label);
                }
        ...
        if (e.keyCode == 13) {
            ...
            $.ajax({
                ...
                success: function (success) {
                    ...
                    if(success) {
                        $('#contact-tag-hint').val('');
                        var tpl = '<li style="background:'+data.bg+'" data-id="'+success+'" id="contact-tag-'+success+'">'+data.term_id+'<span class="dashicons dashicons-no contact-tag-del"></span></li>';
                        $('.contact-assigned-tags').append(tpl);
                        jQuery('.contact-tags-find').slideUp(100);
                    }
	...
	$(document).on('click','.epl-contact-add-new-num',function() {
		...
		<div class="epl-contact-new-num">\
			...
			<select id="epl-contact-sub-select" type="select" class="contact-note-select" > \
	...
	$(document).on('click','.epl-contact-add-new-email',function() {
		...
		<div class="epl-contact-new-email">\
			...
			<select id="epl-contact-sub-mail" type="select" class="contact-note-select" > \
	...
	if( jQuery('.epl-contact-view-more').length == 1 && jQuery('.contact-main-wrapper.left')[0].scrollHeight >= 195){
		jQuery('.epl-contact-view-more').css('display','block');
	}
	jQuery('.epl-contact-view-more').on('click',function() {
		var hight =  jQuery(this).closest('.contact-main-wrapper.left').height();
		if (hight <= 190) {
			jQuery(this).closest('.contact-main-wrapper.left').height('auto');
			jQuery(this).css('position','relative').html('<span class="dashicons dashicons-arrow-right-alt epl-contact-icons"></span>View Less');
		}else {
			jQuery(this).closest('.contact-main-wrapper.left').height('190px');
	...
	if( $('#contact_wp_user').length == 1) {

		var EplUserTimeout = null;

		$('#contact_wp_user').keyup(function() {
			document.body.style.cursor='progress';
			if($('#contact_wp_user').val() == '') {
				...
				$.ajax({
					...
					success: function (response) {
						...
						if( $('.epl-contact-user-suggestion').length == 1 ) {
							...
							$('#contact_wp_user').after(response);
						}
		...
		$(document.body).on('click','.epl-contact-user-suggestion li', function(e) {
			$('#contact_wp_user').val($(this).text());
			$('#contact_wp_user_id').val($(this).data('id'));
			$('.epl-contact-user-suggestion').fadeOut();
		});
	}
	...
	$('.epl-contact-list-table-tags').on('click', '.contact-assigned-tags li', function(e){
