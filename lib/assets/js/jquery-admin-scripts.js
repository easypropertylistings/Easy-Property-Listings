	...
	function($) {
		...
		var EPL_Admin = {
				...
				geocoder.geocode(
					...
					function(results) {
						...
						var map             = new google.maps.Map(
							document.getElementById( 'epl_admin_map_canvas' ),
							{
								zoom: 14,
			...
			init_conditionals : function() {
				...
				var fields = $( '.epl-form-field-wrap input, .epl-form-field-wrap select, .epl-form-field-wrap textarea' );
			...
			handle_epl_tabs_widget : function() {
				if( $( "select[id*='epl_tab_count']" ).length ) {
					var submit 	= $("select[id*='epl_tab_count']").closest('form').find('input[type="submit"]');
					submit.on( 'click',function(e) {
						setTimeout(
							function () {
								$( "select[id*='epl_tab_count']" ).trigger('change');
							},
							500
						);
					});
					$( document ).on( 'change',"select[id*='epl_tab_count']", function(e) {
						var form 	= $(this).closest('form');
						var val 	= $(this).val();
						$tab_titles = form.find( "input[id*='tab_title'], input[id*='tab_id'], textarea[id*='tab_content']" );
						$tab_titles.each( function(index) {
							let id = $(this).attr('id');
		...
		if ( $( '#property_second_agent' ).length == 1 || $( '#property_agent' ).length == 1 ) {
			...
			$( '#property_second_agent,#property_agent' ).keyup(
				function(e) {
					var currTarget             = e.currentTarget;
					document.body.style.cursor = 'progress';
					if ($( currTarget ).val() == '') {
