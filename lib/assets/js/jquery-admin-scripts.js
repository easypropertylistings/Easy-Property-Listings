                                        ...
                                        if ('AND' == relation || 'and' == relation) {
                                                $.each(conditions, function (i, condition) {
                                                       ...
                                                       var el                  = condition[0];
                                                        var operator            = condition[1];
                                                        var value               = condition[2];
                                                        let selector            = '[name="'+el+'"]';
                                                        let selector_el         = $(selector);
                                                        let selector_value      = selector_el.val();

                                                        let is_radio = false;
                                                        let is_checkbox = false;

                                                        if ( selector_el.is(':checkbox') ) {
                                                                is_checkbox     = true;
                                                                selector        = 'input[type="checkbox"][name="' + el + '"]';
                                                        }

                                                        if ( selector_el.is(':radio') ) {
                                                                is_radio        = true;
                                                                selector        = 'input[type="radio"][name="' + el + '"]';
                                                        }

                                                        if( is_checkbox || is_radio ) {
                                                                selector_el             = $(selector);
                                                                selector_value          = selector_el.val();
                                                        }

                                                        if ( selector_el.is(':checkbox') ) {
                                                                selector_value = selector_el.prop('checked') ? selector_value : '';
                                                        }
                                                        ...
                                                        if( selector_el.length ) {
                                                                if ('=' === operator) {
                                                                        should_show = should_show & (value == selector_value );
                                                                } else if ('!=' === operator) {
                                                                        should_show = should_show & (value != selector_value );
                                                                } else if ('>=' === operator) {
                                                                        should_show = should_show & (parseFloat(selector_value) >= value);
                                                                } else if ('<=' === operator) {
                                                                        should_show = should_show & (parseFloat(selector_value) <= value);
                                                                } else if ('>' === operator) {
                                                                        should_show = should_show & (parseFloat(selector_value) > value);
                                                ...
                                                $.each(conditions, function (i, condition) {
                                                        ...
                                                        console.log( [selector, selector_el.length, selector_value, value ] );
