...
function epl_admin_listing_map( $address ) { ?>
	<div id='epl_admin_map_canvas' data-address="<?php echo $address; ?>"></div>
	<?php
}
...
function epl_admin_google_maps_api_notification() {
	...
	if ( 'easy-property-listings_page_epl-settings' === $screen->id ) {
		...
		if ( current_user_can( 'manage_options' ) && 'on' !== epl_get_option( 'epl_disable_google_api' ) && '' === epl_get_option( 'epl_google_api_key' ) ) {
			?>
			<div class="notice notice-error is-dismissible">
				...
				<p><strong><?php _e( 'As of June 22, 2016 Google has made API keys required for the Javascript Maps API.', 'easy-property-listings' ); ?></strong><br>
					...
					<?php
						...
						printf( __( 'To allow maps to function correctly please create a %s and enable <strong>Google Maps API</strong>.', 'easy-property-listings' ), $link );
					?>
					<br>
					...
					<?php
						...
						printf( __( 'Once complete enter your API key below under %s > Google Maps API Key. When set, refresh page to dismiss notice.', 'easy-property-listings' ), $link );
					?>
				</p>
			</div>
			<?php
