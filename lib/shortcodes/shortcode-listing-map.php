...
function epl_shortcode_googlemap_callback( $atts, $content = null ) {
	...
	return '<div class="epl-tab-section epl-section-map">
				<div style="width:' . esc_attr( $atts['width'] ) . '; height:' . esc_attr( $atts['height'] ) . 'px" data-suburb_mode="' . esc_attr( $atts['suburb_mode'] ) . '" data-cord="' . esc_attr( $atts['cord'] ) . '" data-zoom="' . esc_attr( $atts['zoom'] ) . '" data-id="' . esc_attr( $id ) . '" data-address="' . esc_attr( $atts['q'] ) . '" id="epl-default-map">
			</div> </div>';
}
